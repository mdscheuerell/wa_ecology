...
and $\mathbf{C}_e$ and $\mathbf{C}_r$ are $[n \times n]$ matrices of non-zero coefficients (where $n = N_e$ or $n = N_r$).
...
However, because the $O_{r,i}$ are *unknown quantities*, we have no way of estimating $\boldsymbol{\epsilon}_r$, $\boldsymbol{\mu}_r$, or $\boldsymbol{\Sigma}_r$. Thus, one option is to assume that $\boldsymbol{\mu}_r = \boldsymbol{\mu}_e$ and $\boldsymbol{\Sigma}_r  = \boldsymbol{\Sigma}_e$, such that
