...
where $\mathbf{I}$ is an $[n \times n]$ identity matrix. Here, however, it seems more reasonable that 1) the prediction errors $\epsilon_{e,i}$ are not necessarily independent of one one another, 2) the variance of the prediction errors $\epsilon_{e,i}$ is not necessarily constant among the different sampling locations, and 3) $\boldsymbol{\Sigma}_e \neq \boldsymbol{\Sigma}_r$, such that 
...
We can decompose the covariance matrix $\boldsymbol{\Sigma}_e$ in terms of the standard deviations of each of the $\boldsymbol{\epsilon}_i$ $(\sigma_i)$ and the correlations among the $\boldsymbol{\epsilon}_i$ $(\rho_{i,j})$, such that

\begin{equation}
\boldsymbol{\Sigma}_e = 2 \mathbf{S} \mathbf{R} \mathbf{S}
\end{equation}
...
As we understand it, Ecology assumes that $\sigma_i = \sigma ~ \forall ~ i$ and that $\hat{\sigma} \approx 1$ (Table 7 in Ahmed et al. 2019).
