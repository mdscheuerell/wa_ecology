...
We could then use these predictions to estimate both their SD and RMSE (Figure 2). The SD of $\hat{y}$ is ~`r round(sd(yhat), 2)`, but the RMSE is only ~`r round(rmse(yhat, yy), 2)`, which is about `r round(sd(yhat) / rmse(yhat, yy), 0)` times less.
...
`r fig_cap(cnt, caption = "Confidence interval (gray lines) around the expected relationship between $x$ and $y$ (black line).", display = "full")` 
...
Here the paranthetic multiplier on the residual variance $\sigma$ has increased by 1, which creates more uncertainty in the prediction interval than the confidence interval (Figure 4). This is because the CI only needs to account for uncertainty in estimating the expected value of $y$ whereas the PI needs to account for a random future value of $y$.

So, for example, if we wanted to predict what we would observe for $y$ if $x = 10$ with 95% certainty, we would get `r round(y_new[,"fit"], 2)` $\pm$ `r round(y_new[,"fit"] - y_new[,"lwr"], 2)` (Figure 5).
...
`r fig_cap(cnt, caption = "Prediction interval (gray lines) for future unobserved values of $y$.", display = "full")` 
...
`r fig_cap(cnt, caption = "Example of the uncertainty around a new prediction.", display = "full")` 
