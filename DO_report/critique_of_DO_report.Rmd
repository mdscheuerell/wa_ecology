...
We could then use these predictions to estimate both their SD and RMSE (Figure 2). The SD of $\hat{y}$ is ~`r round(sd(yhat), 2)`, but the RMSE is only ~`r round(rmse(yhat, yy), 2)`, which is nearly `r round(sd(yhat) / rmse(yhat, yy), 0)` times less.
...
`r fig_cap(cnt, caption = "Confidence interval (gray lines) around the fitted linear relationship between $x$ and $y$ (black line).", display = "full")` 

In a case like this, however, where we wish to make out-of-sample predictions about some new state of nature, our uncertainty around any single prediction will be greater.
...
`r fig_cap(cnt, caption = "Prediction interval (gray lines) around the fitted linear relationship between $x$ and $y$ (black line).", display = "full")` 
