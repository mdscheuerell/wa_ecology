...
Although we do not know $\text{Cor}(p_{ex}, p_{ref})$, we can estimate it from the calculations on p59. For example, if we assume that  $\text{Var}(\delta) = 0.041$ as for Ecology's model in 2014, then analogous to above we have
...
This correlation is remarkably high, indicating that the two models produce nearly identical predictions of DO. Plugging this correlation coefficient into the equation for $\text{Var}(\delta)$ gives $\text{Var}(\delta) = 2(2.6) (1 - 0.98) = 0.104$, and hence $\text{SD}(\delta) \approx 0.32$. 
